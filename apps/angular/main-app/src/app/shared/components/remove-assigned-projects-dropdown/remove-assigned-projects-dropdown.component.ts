...
@Component({
  selector: 'tms-workspace-remove-assigned-projects-dropdown',
  templateUrl: './remove-assigned-projects-dropdown.component.html',
  styleUrls: ['./remove-assigned-projects-dropdown.component.scss'],
})
export class RemoveAssignedProjectsDropdownComponent {
  ...
  @Output() getSelectedworkspacesArr = new EventEmitter<any>();
  @Output() getSelectedProjectsArr = new EventEmitter<any[]>();
  @Input() prefName = '';
  @Input() userId: any;
  ...
  selectAll() {
    ...
    this.getSelectedworkspacesArr.emit({
      selected_workspace: [],
      not_selected_workspace: [],
    });
    this.getSelectedProjectsArr.emit([]);
  }
  clearAll() {
    ...
    // console.log('%c  this.standloneProjects:', 'color: #0e93e0;background: #aaefe5;', this.standloneProjects);
    ...
    this.getMatTriggerForWorkspaceSelection();
    this.changedSelection = true;
    this.getSelectedworkspaces(this.projectSelectionDataSource.data);
    this.selectedStandloneProjects(this.standloneProjects);
  }
  ...
  itemToggle(checked: boolean, node: WorkSpaceNode) {
    // console.log('this.projectSelectionDataSource: ', this.projectSelectionDataSource.data);
  ...
  getSelectedworkspaces(data: any) {
    const selectedWorkspaceArr: any[] = [];
    const notSelectedWorkspaceArr: any[] = [];
    data?.map((workspace: any) => {
      if (!workspace?.selected) {
        selectedWorkspaceArr.push(workspace);
      } else {
        notSelectedWorkspaceArr.push(workspace);
      }
    });
    this.getSelectedworkspacesArr.emit({
      selected_workspace: selectedWorkspaceArr,
      not_selected_workspace: notSelectedWorkspaceArr,
    });
  }
  ...
  getProjectsData() {
    ...
    if (this.hideNoTaskCount) {
      this.subscriptions.push(
        this.taskService.projectListForUsersActive(Encryption._doEncrypt(this.userId.toString())).subscribe({
          next: (response: any) => {
            if (response) {
              if (response.data) {
  ...
  selectedStandloneProjects(data: any) {
    this.getSelectedProjectsArr.emit(data);
  }
